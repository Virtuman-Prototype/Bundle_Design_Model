...
content_map = {
    ...
    "St. Lawrence River Flow (Water Element)": {
        ...
        "img_persona_b": "https://junaacademie.com/wp-content/uploads/2024/12/untitled-83-1-scaled.jpg", # 建议换成真实的
        "title": "St. Lawrence River Flow",
        "color": "Deep River Blue & Mist",
        "material": "Quick-dry Performance Fabric"
    }
...
# --- 模块 4: 动态输出层------
    ...
    # 1. 获取 M3 选择
    selected_scen = st.session_state.get("scenario_selection", "Laurentian Forest Yoga (Nature Connectivity)")
    current_content = content_map[selected_scen]

    # 2. 模拟后台逻辑：根据 Persona 进行二次排序
    with st.status("🧠 AI Engine: Applying User Persona Re-ranking...", expanded=False):
        st.write(f"👤 Target Persona: **Active Wellness Enthusiast**")
        st.write("📊 Re-calculating weights based on DP1: Behavioral Factors...")
        import time
        time.sleep(1)
        st.write("✅ 3 Optimized Bundles Generated for this Scenario.")

    st.subheader(f"🎯 Dynamic Recommendations for: {current_content['title']}")
    st.caption("Algorithm balances Brand Fit (M1) and Community Trends (M2) with individual Persona.")

    # --- 3. 多卡片展示区域 ---
    # 第一排：主推方案 (Top Pick)
    st.markdown("#### 🏆 Top Pick: Balanced Synergy")
    with st.container(border=True):
        c1, c2 = st.columns([1.2, 1])
        with c1:
            st.image(current_content["img"], use_container_width=True)
        with c2:
            st.info("⭐ **Highest Overall Score: 0.94**")
            st.write(f"**Bundle Name:** {current_content['title']} Elite")
            st.write(f"**Core Logic:** Matches '{current_content['material']}' with current #Hotspots.")
            st.metric("Price", "$249.00", "Bundle Save 15%")
            st.button("📦 Order This Sample", key="main_order")

    st.divider()

    # 第二排：备选方案 (Alternatives based on different Persona weights)
    st.markdown("#### 🔄 Alternative Dynamic Alignments")
    col_alt1, col_alt2 = st.columns(2)

    with col_alt1:
        with st.container(border=True):
            st.write("🌿 **Persona A: Eco-Conscious**")
            # 这里可以放一张稍微不同的图或者用同一张图强调不同点
            st.image(current_content["img_persona_a"], use_container_width=True) 
            st.caption("Focus: Maximize Sustainable Materials")
            st.write("**Score:** 0.88")
            if st.button("View Details", key="alt1"):
                st.toast("Switching to Eco-weighted logic...")

    with col_alt2:
        with st.container(border=True):
            st.write("⚡ **Persona B: Trend-Driven**")
            st.image(current_content["img_persona_b"], use_container_width=True)
            st.caption("Focus: Maximize Social Media Visibility")
            st.write("**Score:** 0.82")
            if st.button("View Details", key="alt2"):
                st.toast("Switching to Trend-weighted logic...")

    # --- 4. 反馈闭环 (DR12) ---
    st.sidebar.markdown("---")
    st.sidebar.write("🔄 **Feedback Loop (DR12)**")
    feedback = st.sidebar.select_slider("Rate this Re-ranking Alignment", options=["Poor", "Fair", "Good", "Excellent"])
    if st.sidebar.button("Submit to M1/M2"):
        st.sidebar.success("Iteration Logic Updated!")
